...
void iptables_rule()  // handles -a -g --gen-add  --keep-rule --clear --wait-lock
{
    ...
    if (program_mode == client_mode) {
        ...
        if (raw_mode == mode_faketcp) {
            snprintf(tmp_pattern, sizeof(tmp_pattern), "-s %s -p tcp -m tcp --sport %d", remote_addr.get_ip(), remote_addr.get_port());
        }
        if (raw_mode == mode_udp) {
            snprintf(tmp_pattern, sizeof(tmp_pattern), "-s %s -p udp -m udp --sport %d", remote_addr.get_ip(), remote_addr.get_port());
        }
        if (raw_mode == mode_icmp) {
            if (raw_ip_version == AF_INET)
                snprintf(tmp_pattern, sizeof(tmp_pattern), "-s %s -p icmp --icmp-type 0", remote_addr.get_ip());
            else
                snprintf(tmp_pattern, sizeof(tmp_pattern), "-s %s -p icmpv6 --icmpv6-type 129", remote_addr.get_ip());
        }
        pattern += tmp_pattern;
    }
    if (program_mode == server_mode) {
        ...
        if (raw_ip_version == AF_INET) {
            if (local_addr.inner.ipv4.sin_addr.s_addr != 0) {
                snprintf(tmp_pattern, sizeof(tmp_pattern), "-d %s ", local_addr.get_ip());
            }
        } else {
            char zero_arr[16] = {0};
        ...
        tmp_pattern[0] = 0;
        if (raw_mode == mode_faketcp) {
            snprintf(tmp_pattern, sizeof(tmp_pattern), "-p tcp -m tcp --dport %d", local_addr.get_port());
        }
        if (raw_mode == mode_udp) {
            snprintf(tmp_pattern, sizeof(tmp_pattern), "-p udp -m udp --dport %d", local_addr.get_port());
        }
        if (raw_mode == mode_icmp) {
            if (raw_ip_version == AF_INET)
                snprintf(tmp_pattern, sizeof(tmp_pattern), "-p icmp --icmp-type 8");
            else
                snprintf(tmp_pattern, sizeof(tmp_pattern), "-p icmpv6 --icmpv6-type 128");
        }
        pattern += tmp_pattern;
    }
    /*
